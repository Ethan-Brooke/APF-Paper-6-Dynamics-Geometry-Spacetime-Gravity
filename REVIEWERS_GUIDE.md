# Reviewers' Guide — APF Paper 6: Dynamics and Geometry as Optimal Admissible Reallocation

This guide is the physics-first walkthrough for peer reviewers of Dynamics and Geometry as Optimal Admissible Reallocation. It addresses the structural assumptions, anticipated objections, and falsification surfaces of the paper's argument. The intended audience is a peer reviewer with a background in physics or mathematics.

The guide complements three other verification routes:
- The [executable codebase](apf/) (run `python run_checks.py` for the full 22-theorem verification);
- The [Colab notebook](APF_Reviewer_Walkthrough.ipynb) (zero-install, theorem-by-theorem cells with prose + code);
- The [interactive derivation DAG](https://ethan-brooke.github.io/Dynamics-Geometry-Spacetime-Gravity/) (hover-for-details, click-for-dependencies, animated verification).

---

## The derivation in 22 steps

The full chain from the foundational commitments to this paper's results:

**Step 1 — Regime_exit_Type_II** (`check_Regime_exit_Type_II`)

  Regime_exit_Type_II: Minimizer Nonuniqueness (Branching) [P].


**Step 2 — Regime_exit_Type_III** (`check_Regime_exit_Type_III`)

  Regime_exit_Type_III: Change of Admissible Class (Record Locking) [P].


**Step 3 — L_irr** (`check_L_irr`)

  L_irr: Irreversibility from Admissibility Physics.


**Step 4 — L_Gleason_finite** (`check_L_Gleason_finite`)

  L_Gleason_finite: Born Rule from Frame Function (Finite-Dim) [P].


**Step 5 — T_Bek** (`check_T_Bek`)

  T_Bek: Bekenstein Bound from Interface Capacity.


**Step 6 — Regime_exit_Type_I** (`check_Regime_exit_Type_I`)

  Regime_exit_Type_I: Collapse of Admissible Variation (Saturation) [P].


**Step 7 — Regime_exit_Type_IV** (`check_Regime_exit_Type_IV`)

  Regime_exit_Type_IV: Loss of Smooth or Local Structure [P].


**Step 8 — Regime_exit_Type_V** (`check_Regime_exit_Type_V`)

  Regime_exit_Type_V: Pure Representational Redundancy [P].


**Step 9 — T7B** (`check_T7B`)

  T7B: Metric Uniqueness from Polarization Identity.


**Step 10 — L_HKM_causal_geometry** (`check_L_HKM_causal_geometry`)

  L_HKM_causal_geometry: Causal Order Determines Conformal Geometry [P].


**Step 11 — L_Malament_uniqueness** (`check_L_Malament_uniqueness`)
...
To prevent scope inflation, the following are flagged explicitly as *outside* the scope of Dynamics Geometry Spacetime Gravity:
...
- **`apf/core.py`** — the 22 theorem check functions for this paper. Each function constructs a mathematical witness, verifies the theorem's claim, and returns a structured result with name, dependencies, status, and key result.
...
This repository is the executable mathematical appendix to APF Paper 6. The canonical archival deposit is at [https://doi.org/10.5281/zenodo.18439445](https://doi.org/10.5281/zenodo.18439445) (DOI: 10.5281/zenodo.18439445).
